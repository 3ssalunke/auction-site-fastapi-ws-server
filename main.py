...
class AuctionConnectionManager:
    ...
    async def connect(self, websocket: WebSocket, auction_id):
        ...
        if(item_found.ends and item_found.ends < datetime.datetime.now()) or item_found.completed:
    ...
    async def send_personal_message(self, message: str, websocket: WebSocket, cur_price=None, json_data=None):
    ...
    try:
        while True:
            ...
            if item.min_price <= new_bid >= min_new_bid:
                ...
                session.commit()
                await manager.broadcast(f"Participant {participant_id} has bid {item.bid}!", auction_id=id, new_price=item.bid, ends=item.ends)
